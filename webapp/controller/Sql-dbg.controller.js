sap.ui.define([
	...
	return BaseController.extend("adrien_ratsimihah_portfolio.controller.Sql", {
		...
		getSkillData: function() {
			...
			return new Promise(function(resolve, reject) {
				...
				techModel.attachRequestCompleted(function(oEvent) {
					//Titre page
					data = techModel.getProperty("/sql_titre");
					...
					//Detail 
					data = techModel.getProperty("/sql_detail");
					partie = self._base.removeVirgule(data);
					self.getView().byId("sql_detail").setHtmlText(partie);
				...
				realisationModel.attachRequestCompleted(function(oEvent) {
					...
					if (oEvent.getSource().getData().Realisations instanceof Array) {
						for (i = 0; i <= oEvent.getSource().getData().Realisations.length - 1; i++) {
							if (oEvent.getSource().getData().Realisations[i].techno.indexOf("SQL") !== -1) {
