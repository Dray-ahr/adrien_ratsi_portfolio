sap.ui.define([
	...
	return BaseController.extend("adrien_ratsimihah_portfolio.controller.JavaPhp", {
		...
		onInit: function() {
			...
			this.nomfragment = "javaphp_fragment";
		...
		getSkillData: function() {
			...
			return new Promise(function(resolve, reject) {
				...
				techModel.attachRequestCompleted(function(oEvent) {
					...
					//Titre page
					data = techModel.getProperty("/javaphp_titre");
					self.getView().byId("pageTitle").setTitle(data);

					//Detail 
					data = techModel.getProperty("/javaphp_detail");
					partie = self._base.removeVirgule(data);
					self.getView().byId("javaphp_detail").setHtmlText(partie);
				...
				realisationModel.attachRequestCompleted(function(oEvent) {
					...
					if (oEvent.getSource().getData().Realisations instanceof Array) {
						for (i = 0; i <= oEvent.getSource().getData().Realisations.length - 1; i++) {
							if (oEvent.getSource().getData().Realisations[i].techno.indexOf("JAVAPHP") !== -1) {
