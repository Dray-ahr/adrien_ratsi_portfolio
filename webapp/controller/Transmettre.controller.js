sap.ui.define([
	...
	return BaseController.extend("adrien_ratsimihah_portfolio.controller.Transmettre", {
		...
		onInit: function() {
			...
			this.nomfragment = "transmettre_fragment";
		...
		getSkillData: function() {
			...
			return new Promise(function(resolve, reject) {
				...
				humModel.attachRequestCompleted(function(oEvent) {
					...
					//Titre page
					data = humModel.getProperty("/Transmettre/TITRE");
					self.getView().byId("pageTitle").setTitle(data);

					//Titre 1
					data = humModel.getProperty("/Transmettre/titre1");
					self.getView().byId("titre1").setHtmlText(data);

					//Partie 1
					data = "";
					data = humModel.getProperty("/Transmettre/partie1");
					partie = self._base.removeVirgule(data);
					self.getView().byId("partie1").setHtmlText(partie);

					//Titre 2
					data = humModel.getProperty("/Transmettre/titre2");
					self.getView().byId("titre2").setHtmlText(data);

					//Partie 2
					partie = "";
					data = " ";
					data = humModel.getProperty("/Transmettre/partie2");
					partie = self._base.removeVirgule(data);
					self.getView().byId("partie2").setHtmlText(partie);

					//Titre 3
					data = humModel.getProperty("/Transmettre/titre3");
					self.getView().byId("titre3").setHtmlText(data);

					//Partie 3
					partie = "";
					data = " ";
					data = humModel.getProperty("/Transmettre/partie3");
					partie = self._base.removeVirgule(data);
				...
				realisationModel.attachRequestCompleted(function(oEvent) {
					...
					if (oEvent.getSource().getData().Realisations instanceof Array) {
						for (i = 0; i <= oEvent.getSource().getData().Realisations.length - 1; i++) {
							if (oEvent.getSource().getData().Realisations[i].techno.indexOf("Transmettre") !== -1) {
