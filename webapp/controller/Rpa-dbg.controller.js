sap.ui.define([
	...
	return BaseController.extend("adrien_ratsimihah_portfolio.controller.Rpa", {
		...
		getSkillData: function() {
			...
			return new Promise(function(resolve, reject) {
				...
				techModel.attachRequestCompleted(function(oEvent) {
					...
					//Titre page
					data = techModel.getProperty("/rpa_titre");
					...
					//Detail 
					data = techModel.getProperty("/rpa_detail");
					partie = self._base.removeVirgule(data);
					self.getView().byId("rpa_detail").setHtmlText(partie);
				...
				realisationModel.attachRequestCompleted(function(oEvent) {
					...
					if (oEvent.getSource().getData().Realisations instanceof Array) {
						for (i = 0; i <= oEvent.getSource().getData().Realisations.length - 1; i++) {
							if (oEvent.getSource().getData().Realisations[i].techno.indexOf("RPA") !== -1) {
